...
def CNN_model():
    model = Sequential()
    # define CNN model
    model.add(Conv2D(bs, (3, 3), strides=1, activation = 'relu' ,input_shape = (50,50,1)))
    model.add(MaxPooling2D(pool_size=(3,3)))
    
    model.add(Conv2D(bs, (3, 3), strides=1, activation = 'relu'))
    model.add(MaxPooling2D(pool_size=(3,3)))
...
for train, test in kf:
    ...
    with open("results_third.txt", "a") as f:
        print(report, end="\n\n", file=f)
